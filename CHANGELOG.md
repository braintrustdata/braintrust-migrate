...
- Make same-name project reuse explicit so it isn't mistaken for an overwrite. The destination project is resolved by name: when a project with the matching name already exists, resources are migrated **into** it (additively — nothing is deleted or replaced) rather than creating a new project. This is intentional and what makes re-runs idempotent, but it was only logged at `debug`. It is now surfaced as a `warning` at migration time, as a one-line yellow notice under each affected project header in the live run, and as a caption under the `--dry-run` "Project Migration Plan" table whenever a destination project shows status `exists`. Each message points to `--project-map` for migrating into a new, separate project. Behavior is unchanged; only visibility. A code function's compiled bundle is produced by the push/eval build pipeline and is not exposed by the API, so it can't be recreated in the destination — migrating it produces a broken function. These are now skipped (recorded with `skip_reason="code_bundle_not_migratable"` and logged with name/slug so they can be re-pushed manually). Inline code functions (which carry their source) and all other function types continue to migrate normally.
