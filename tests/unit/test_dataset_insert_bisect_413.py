        ...
        *,
        ...
        if path.endswith("/insert"):
            ...
            if len(events) > self.max_events_per_insert:
                req = httpx.Request(
                    "POST", "https://api.braintrust.dev/v1/dataset/x/insert"
                )
...
@pytest.mark.asyncio
async def test_dataset_insert_bisects_on_413(tmp_path: Path) -> None:
    TOTAL_EVENTS = 25
    MAX_PER_INSERT = 10
    ds_id = "ds-source"
    ...
    await migrator._migrate_dataset_records_streaming(ds_id, "ds-dest")
